"""Exclusive edit-lock for Ventas de Granos (FR-012b/FR-012c), backed by
`dbo.VentaGranosEditLocks` en `WC`. Copia exacta del patrón de
`ventas_hacienda/repository_locks.py` / `compras/repository_locks.py`.
"""
...
def _get_lock_row(id_venta: int) -> dict | None:
    return fetch_one(
        ...
        "FROM dbo.VentaGranosEditLocks WHERE IdVenta = ?",
        (id_venta,),
    )
...
def adquirir_lock(id_venta: int, lock_token: str, force: bool = False) -> LockInfo | None:
    ...
    if row is None:
        execute_write(
            "INSERT INTO dbo.VentaGranosEditLocks (IdVenta, LockToken, LockedAt, ExpiresAt) "
            "VALUES (?, ?, ?, ?)",
            (id_venta, lock_token, now, expires_at),
        )
    else:
        execute_write(
            "UPDATE dbo.VentaGranosEditLocks SET LockToken = ?, LockedAt = ?, ExpiresAt = ? "
            "WHERE IdVenta = ?",
...
def liberar_lock(id_venta: int, lock_token: str) -> bool:
    ...
    if _is_vigente(row) and not _mismo_token(row, lock_token):
        return False
    execute_write("DELETE FROM dbo.VentaGranosEditLocks WHERE IdVenta = ?", (id_venta,))
    return True
