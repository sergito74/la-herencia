...
- **FR-001**: El sistema MUST mostrar, como pantalla de entrada del módulo, el resultado consolidado de una Campaña completa (todos sus Cultivos juntos) con una tabla resumen de una fila por Cultivo (Historia 1).
...
- **FR-003**: El sistema MUST mostrar, por Cultivo/Campaña (y consolidado por Campaña): superficie sembrada, superficie cosechada, superficie picada, rinde, costo total, costo por hectárea (sembrada y cosechada), venta neta, margen bruto y rentabilidad — cada monto en pesos y en dólares, calculados como dos series independientes (cada una a partir del TC histórico de cada línea de origen; no se validan ni recalculan dividiendo una por la otra, ver Assumptions).
...
- **FR-006**: El sistema MUST traducir cada Cultivo a su `IdDestino` (vistas de costeo) y a su `IdGrano` (vistas de venta) vía `Map_CultivoResultado`, y traducir `Campaña` (texto) a `IdCampaña` (numérico) vía la tabla `Campañas`, antes de unir las distintas fuentes.
...
- **FR-010**: El sistema MUST señalar con un badge ocre y tooltip explicativo (no bloqueante, no rojo de error) cuando el costeo heredado de un Cultivo/Campaña está incompleto por falta de mapeo en `Map_CultivoResultado`.
- **FR-011**: El sistema MUST señalar con un badge/ícono de advertencia (no bloqueante) cuando un Cultivo/Campaña tiene venta registrada y costo total en cero (o muy por debajo de lo esperable), indicando "costos incompletos — margen no representativo".
