...
- **`/speckit-clarify` (2026-09-22, segunda pasada)**: 2 preguntas formales resueltas — campaña por defecto al entrar al módulo (FR-001: campaña "actual" derivada del nombre, con resguardo a la más reciente con datos) y umbral de la advertencia de costos incompletos (FR-011: costo total < 20% de la venta neta). Se agregó también, sin gastar pregunta, el caso de superficie cero al calcular costo por hectárea (mismo criterio que el rinde).
- **`/speckit-analyze` (2026-09-22)**, después de `/speckit-plan` y `/speckit-tasks`, con remediación aplicada a `spec.md`/`data-model.md`/`plan.md`/`tasks.md`/`quickstart.md`/`contracts/`:
  - **I1 (HIGH)**: FR-010 estaba atado a un `IdDestino` huérfano, un caso que nunca ocurre a nivel de un Cultivo real (`Map_CultivoResultado` es 1:1) — redefinido sobre la superficie cosechada auto-estimada de `ResultadoCultivo_Cierre` (41 de 43 filas reales), un caso real y por Cultivo/Campaña.
  - **G1 (HIGH)**: FR-006 solo traducía Campaña texto→id; las vistas de venta necesitan la dirección inversa (id→texto) — agregada `campania_id_a_texto`.
  - **G2/U1/U2/U3/G3/C1 (MEDIUM/LOW)**: agregado test de `mapeo.py` (T011), excluido el rinde del consolidado de Campaña por no ser comparable entre Cultivos (FR-003), agregado el filtro directo de Cultivo como segundo camino de FR-002 (T032), agregado el estado vacío a nivel de Cultivo (T029), y agregados a `quickstart.md` el paso de "sin reapertura" (SC-003) y la repetición en 5/3 combinaciones reales (SC-002/SC-004).
