...
- [X] T086 Correr manualmente los 10 escenarios de `specs/007-ventas-hacienda-granos/quickstart.md` contra el backend real (`WC`) y verificar contra SQL Server que ninguna escritura llegó a `LaHerencia` (SC-004). Ejecutado 2026-09-18 contra backend real en `WC`: Escenarios 1/2 (alta con 2 líneas de compradores distintos, subTotal/importe correctos), 3 (lock A→200, lock B sin force→409, lock B force→200, PUT recalcula totales, DELETE 204 sin dejar cabecera/líneas/vencimientos), 4 (vencimiento restante conserva su importe tras editar), 5 (documento duplicado → 201 con `warnings`, no bloqueante), 6 (`/api/ventas-granos` sin filtro → `items: []`; con `campania=2025/2026` → resultados reales), 7 (alta Granos con ajuste+deducción, `importeNetoAPercibir` verificado a mano, ajustes/deducciones devueltos por separado), 8 (PUT agrega deducción y baja `importeNetoAPercibir` en el monto exacto; DELETE 204 sin huérfanos en Ajustes/Deducciones), 10 (confirmado por código: submenú Ventas > Hacienda/Granos en `NavHeader.tsx`, botones "+ Nueva venta de..." en `/ventas/hacienda` y `/ventas/granos`). Verificado con `IdVenta=1149` (Hacienda) e `IdVenta=229` (Granos, creadas y luego borradas por este test) que la fila existía en `WC.dbo.[Venta Hacienda]` y no en `LaHerencia.dbo.[Venta Hacienda]` vía conexión directa de solo lectura a ambas bases.
- [X] T087 Ejecutar el Escenario 9 de `quickstart.md` (SC-002/SC-003): tomar 5 `IdVenta` reales de `Venta Hacienda` y 5 de `Venta Granos`, recalcular manualmente sus totales con las fórmulas de data-model.md y confirmar que coinciden con lo que produce `calcular_totales`; documentar el resultado (mismo criterio que el Escenario 7/T040a de 006-carga-compras). Ejecutado 2026-09-18: Hacienda `IdVenta` 143/144/145/146/147 y Granos `IdVenta` 224/225/226/227/228 (todas históricas reales en `WC`, no creadas por este test). Recalculado a mano `subTotal`/`comision`/`iva`/`importe`/`importeTotal` (Hacienda) y `precioKg`/`subTotal`/`iva`/`totalOperacion`/`totalDeducciones`/`importeNetoAPercibir` (Granos) con las fórmulas exactas de `data-model.md`, comparado contra `GET /api/ventas-hacienda/{id}` y `GET /api/ventas-granos/{id}` reales — coincidencia exacta en los 10 casos (las únicas diferencias observadas fueron de redondeo en la representación decimal, no del valor).
