...
- **FR-011**: Cualquier usuario con permiso de escritura DEBE poder anular una aplicación existente, sin restricción de rol adicional a la que el sistema ya aplica globalmente (016-autenticacion: los usuarios con rol `Lectura` no pueden hacer escrituras en ninguna parte del sistema, esta feature no es una excepción a esa regla).
...
- Anulación de aplicaciones: sin restricción de rol propia de esta feature — cualquier usuario con permiso de escritura puede hacerlo (confirmado por el dueño), sujeto a la regla global ya vigente de que los usuarios `Lectura` no escriben en ningún módulo del sistema.
